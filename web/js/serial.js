(function () {
    ...
    e.AmSerialChart = e.Class({inherits: e.AmRectangularChart, construct: function (a) {
            this.type = "serial";
            ...
            if (b) {
                ...
                for (var m = 0; m < c.length; m++) {
                    for (var l = c[m], k = 0, p = 0, n, x, A, u, v, t = 0, q = 0, r, w, z, B, F, G = this.start; G <= this.end; G++) {
                        ...
                        if (y && (y = y.axes[g.id].graphs[b.id])) {
                            if (y.values) {
                                var C = y.values[l];
                                if (this.rotate) {
                                    if (0 > y.x || y.x > y.graph.height)
                                        C = NaN
                                } else if (0 > y.x || y.x > y.graph.width)
                                    C = NaN;
                                if (!isNaN(C)) {
                                    isNaN(n) && (n = C);
                                    x = C;
                    ...
                    a = e.formatValue(a, {open: n, close: x, high: u, low: A, average: v, sum: k, count: p}, d, f, l + "\\.", this.usePrefixes, this.prefixesOfSmallNumbers, this.prefixesOfBigNumbers);
...
(function () {
    var e = window.AmCharts;
    e.Cuboid = e.Class({construct: function (a, b, c, d, e, h, f, m, l, k, p, n, x, A, u, v, t) {
            this.set = a.set();
            this.container = a;
            this.h = Math.round(c);
            this.w = Math.round(b);
            ...
            var a = this.set;
            a.clear();
            var b = this.container, c = b.chart, d = this.w, g = this.h, h = this.dx, f = this.dy, m = this.colors, l = this.alpha, k = this.bwidth, p = this.bcolor, n = this.balpha, x = this.gradientRotation, A = this.cornerRadius, u = this.dashLength, v = this.pattern, t = this.topRadius, q = this.bcn, r = m, w = m;
            ...
            1 > Math.abs(g) && (g = 0);
            1 > Math.abs(d) && (d = 0);
            !isNaN(t) && (0 < Math.abs(h) || 0 < Math.abs(f)) ? (m = [r], m = {fill: m, stroke: p, "stroke-width": k, "stroke-opacity": n, "fill-opacity": l}, K ? (l = "M0,0 L" + d + "," + (g / 2 - g / 2 * t), k = " B", 0 < d && (k = " A"), e.VML ? (l += k + Math.round(d -
            ...
            for (z = 0; z < d.length; z++)
                if (B = d[z])
                    a.push(B), e.setCN(c, B, q + "element");
            v && b.pattern(v)
        }, width: function (a) {
            isNaN(a) && (a = 0);
            this.w = Math.round(a);
...
(function () {
    var e = window.AmCharts;
    e.CategoryAxis = e.Class({inherits: e.AxisBase, construct: function (a) {
            ...
            if (e.ifArray(a)) {
                ...
                var d = this.start, g = this.labelFrequency, h = 0, f = this.end - d + 1, m = this.gridCountR, l = this.showFirstLabel, k = this.showLastLabel, p, n = "", n = e.extractPeriod(this.minPeriod), x = e.getPeriodDuration(n.period, n.count), A, u, v, t, q;
                A = this.rotate;
                ...
                if (this.parseDates && !this.equalSpacing)
                    this.lastTime = a[a.length - 1].time, this.maxTime = e.resetDateToMin(new Date(this.lastTime + 1.05 * x), this.minPeriod, 1, b).getTime(), this.timeDifference = this.endTime - this.startTime, this.parseDatesDraw();
                        ...
                        for (b = f; b <= this.end + 2; b++) {
                            ...
                            if (q && !isNaN(w))
                                if (b / w == Math.round(b / w) || p)
                                    b / m == Math.round(b / m) || p || (this.gridAlpha = this.minorGridAlpha, n = void 0);
                                else
                                    continue;
                    ...
                    if (0 < this.gridCountR)
                        for (b = f; b < a; b++)
                            if (t = this.data[b].time, this.checkPeriodChange(g, A, t, n) && b >= F) {
                                ...
                                isNaN(w) || (this.checkPeriodChange(g, w, t, z) && (this.gridAlpha = this.minorGridAlpha, f = this.getCoordinate(b - this.start), q = new this.axisItemRenderer(this, f), this.pushAxisItem(q), z = t), this.gridAlpha = r)
                }
                for (b = 0; b < this.data.length; b++)
                    if (l = this.data[b])
                        k = this.parseDates && !this.equalSpacing ? Math.round((l.time - this.startTime) * this.stepWidth + this.cellWidth / 2) : this.getCoordinate(b - h), l.x[this.id] = k;
